...
else: random1obj = AudioSegment.from_mp3(os.path.join(str(drums), random1)) * 8
...
else: random2obj = AudioSegment.from_mp3(os.path.join(str(bass), random2)) * 8
...
else: random4obj = AudioSegment.from_mp3(os.path.join(str(pads), random4)) * 8
...
combined1 = random1obj.overlay(random2obj + phrase_of_silence + random2obj * 6 , position=fourbar) #WHY DOESNT THIS WORK??!!?
combined2 = combined1.overlay(random3obj, position=eightbar) 
combined3 = combined2.overlay(random4obj, position=sixteenbar)
